...
if inlines[0].lower().strip() == "#nexus":
    filetype = "trees"
# This assumes that log files are always longer than 1000 lines,
# while lists always include less than 1000 files to be combined.
elif len(inlines) < 1000:
    filetype = "list"
...
for inline in inlines:
    if filetype == "log":
        ...
        if inline[:6].lower() == "sample":
            break
    elif filetype == "trees":
        if inline[:4].lower() == "tree":
            break
        comments_string += inline + "\n"
    else:
...
for x in range(len(names_of_files_to_be_read)+1):
    ...
    if filetype == "log":
        ...
        if comments_string_this_file != comments_string:
            print("ERROR: File " + names_of_files_to_be_read[x-1] + "has a different header than the first file in the list. These may not be combinable.")
            sys.exit(1)
        for inline in inlines:
