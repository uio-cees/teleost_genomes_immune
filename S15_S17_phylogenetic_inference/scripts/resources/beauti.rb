...
if ucln_clock
    if starbeast
        ...
        xml_string << "#{first_tab}\t\t<!-- Parameters of the UCLN clock -->\n"
...
if ucln_clock
    if starbeast
        xml_string << "#{first_tab}\t\t\t<!-- An exponential prior on the standard deviation of the rate of each uncorrelated lognormal clock -->\n"
        gene_tree_ids.each do |g|
        ...
        xml_string << "#{first_tab}\t\t\t<!-- An exponential prior on the standard deviation of the rate of the uncorrelated lognormal clock -->\n"
        xml_string << "#{first_tab}\t\t\t<prior id=\"ucldStdevPrior.c:combined\" name=\"distribution\" x=\"@ucldStdev.c:Species\">\n"
        xml_string << "#{first_tab}\t\t\t\t<Exponential name=\"distr\" mean=\"0.3333\"/>\n"
        xml_string << "#{first_tab}\t\t\t</prior>\n"
    end
end
...
alignment_ids.each do |a|
    ...
    if ucln_clock
        ...
        xml_string << "#{first_tab}\t\t\t\t<branchRateModel id=\"strictClock.c:#{tree_id}\" clock.rate=\"1.0\" spec=\"beast.evolution.branchratemodel.StrictClockModel\"/>\n"
    end
    xml_string << "#{first_tab}\t\t\t</distribution>\n"
end
xml_string << "\n"
...
if write_slurm
    ...
    slurm_string1 << "fi\n"
    slurm_string1 << "if [ -f #{analysis_id}_* ]; then\n"
    slurm_string1 << "  cp #{analysis_id}_* $SCRATCH\n"
    slurm_string1 << "fi\n"
    ...
    random = rand(100000)
    slurm_string_start = "java -jar -Xmx4g beast.jar -threads 3 -seed #{random} -beagle #{analysis_id}.xml\n"
    slurm_string_resume = "java -jar -Xmx4g beast.jar -threads 3 -seed #{random} -beagle -resume #{analysis_id}.xml\n"
    slurm_string2 = ""
    slurm_string2 << "\n"
